...
class _MakanPageState extends State<MakanPage> {
  ...
  int? _editId; // Variabel untuk mendeteksi apakah sedang mode Edit
  ...
  void _editData(Map<String, dynamic> data) {
    setState(() {
      _editId = data['id'];
      _tanggalDipilih = DateTime.parse(data['tanggal']);
  ...
  void _hapusData(int id) async {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Catatan pengeluaran makan ini akan dihapus secara permanen.'),
        actions: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Biaya Makan'), backgroundColor: Colors.orange[800], foregroundColor: Colors.white),
      body: Column(
        children: [
          // PANEL INPUT / EDIT
          Padding(
            ...
            child: Card(
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    // Fitur Ubah Tanggal (Berguna jika lupa catat hari kemarin)
                    Row(
                      ...
                      children: [
                        ...
                        TextButton.icon(
                          ...
                          icon: const Icon(Icons.calendar_month, size: 18, color: Colors.orange),
                          label: const Text('Ubah', style: TextStyle(color: Colors.orange)),
                        )
                      ],
                    ),
                    const Divider(),
                    ...
                    Row(
                      children: [
                        if (_editId != null) // Tombol Batal Edit (Hanya muncul jika sedang edit)
                          Expanded(
                        ...
                        Expanded(
                          ...
                          child: ElevatedButton(
                            style: ElevatedButton.styleFrom(backgroundColor: _editId == null ? Colors.orange[800] : Colors.green[700], foregroundColor: Colors.white),
                            onPressed: _simpanMakan, 
                            child: Text(_editId == null ? 'SIMPAN PENGELUARAN' : 'UPDATE DATA')
                          ),
                        ),
                      ],
                    ),
          ...
          const Padding(
            ...
            child: Align(alignment: Alignment.centerLeft, child: Text('Riwayat Pembelian Nasi', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey))),
          ),

          // DAFTAR RIWAYAT DENGAN MENU EDIT/HAPUS
          Expanded(
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      color: _editId == m['id'] ? Colors.orange[50] : Colors.white, // Sorot warna jika sedang diedit
                      child: ListTile(
                        ...
                        // KODE BARU: Tombol Popup Menu (Titik Tiga)
                        trailing: Row(
                          ...
                          children: [
                            ...
                            PopupMenuButton<String>(
                              onSelected: (value) {
                                if (value == 'edit') _editData(m);
                                if (value == 'hapus') _hapusData(m['id']);
                              },
                              itemBuilder: (context) => [
                                const PopupMenuItem(value: 'edit', child: Row(children: [Icon(Icons.edit, color: Colors.orange, size: 20), SizedBox(width: 8), Text('Edit Data')])),
                                const PopupMenuItem(value: 'hapus', child: Row(children: [Icon(Icons.delete, color: Colors.red, size: 20), SizedBox(width: 8), Text('Hapus')])),
                              ],
                            ),
                          ],
                        ),
